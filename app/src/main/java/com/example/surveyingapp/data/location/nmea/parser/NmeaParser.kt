...
class NmeaParser {
    ...
    // Keep MSL explicit; derive ellipsoidal from MSL + geoid when needed
    data class Gga(
        val timeRaw: String?,
        val lat: Double?,
        val lon: Double?,
        ...
        val sats: Int?,
        val hdop: Double?,
        val altMsl: Double?,                 // GGA field 9 = MSL
        val geoidSeparation: Double?,        // GGA field 11
        val diffAgeSec: Double?,
        val stationId: String?
    ) {
        // Back-compat helpers
        val alt: Double? get() = altMsl
        val altEllipsoidal: Double? get() = if (altMsl != null && geoidSeparation != null) altMsl + geoidSeparation else null
        val ellipsoidalHeight: Double? get() = altEllipsoidal
    }
    ...
    data class Gsa(val pdop: Double?, val hdop: Double?, val vdop: Double?, val usedSvids: List<Int> = emptyList())
        ...
        val sentence: NmeaSentence? = when (type) {
            ...
            "GPRMC","GNRMC","GARMC","GLRMC","GBRMC" ->
                parseRmc(parts)?.let { NmeaSentence.RmcSentence(it) }
                    ?: return ParseResult.Dropped(ParseResult.Dropped.Reason.INACTIVE_STATUS)
    ...
    private fun parseGga(p: List<String>): Gga? {
        ...
        val lat = decodeLat(p.getOrNull(2), p.getOrNull(3))
        val lon = decodeLon(p.getOrNull(4), p.getOrNull(5))
        val fixQ = p.getOrNull(6)?.toIntOrNull()
        val sats = p.getOrNull(7)?.toIntOrNull()
        val hdop = p.getOrNull(8)?.toDoubleOrNull()
        val altMsl = p.getOrNull(9)?.toDoubleOrNull()           // field 9 = MSL
        val geoidSep = p.getOrNull(11)?.toDoubleOrNull()         // field 11
    ...
    private fun parseRmc(p: List<String>): Rmc? {
        ...
        if (p[2] != "A") return null // active only
        val timeRaw = p[1].ifBlank { null }
        val lat = decodeLat(p.getOrNull(3), p.getOrNull(4))
        val lon = decodeLon(p.getOrNull(5), p.getOrNull(6))
        val speed = p.getOrNull(7)?.toDoubleOrNull()
        val course = p.getOrNull(8)?.toDoubleOrNull()
        val date = p.getOrNull(9)?.ifBlank { null }
        return Rmc(timeRaw, date, lat, lon, speed, course)
    }

    private fun parseGsv(p: List<String>, constellationHint: Constellation): Gsv? {
        if (p.size < 4) return null
        val total = p.getOrNull(1)?.toIntOrNull()
        val num   = p.getOrNull(2)?.toIntOrNull()
        val inViewField = p.getOrNull(3)?.toIntOrNull()

        val satList = mutableListOf<Satellite>()
        var idx = 4
        while (idx + 3 < p.size) {
            val prn  = p.getOrNull(idx    )?.toIntOrNull()
            val elev = p.getOrNull(idx + 1)?.toIntOrNull()
            val az   = p.getOrNull(idx + 2)?.toIntOrNull()
            val snr  = p.getOrNull(idx + 3)?.toIntOrNull()

            // Include ALL satellites that have a PRN, even if missing other data
            if (prn != null && prn > 0) {
                val constell = if (constellationHint != Constellation.UNKNOWN)
                    constellationHint
                else
                    inferConstellationBySvid(prn) // infer for GN talker
                satList.add(Satellite(prn, constell, elev, az, snr))
                android.util.Log.d("GSV_PARSE", "Added satellite: PRN=$prn, constellation=$constell, elev=$elev, az=$az, snr=$snr")
            }
            idx += 4
        }

        // Prefer actual parsed satellite count over the reported count
        val actualSatsInView = maxOf(satList.size, inViewField ?: 0)

        android.util.Log.d("GSV_PARSE", "GSV parsed: totalMsgs=$total, msgNum=$num, reportedInView=$inViewField, actualSats=${satList.size}, finalInView=$actualSatsInView")

        return Gsv(total, num, actualSatsInView, satList)
    }

    private fun parseGsa(p: List<String>): Gsa? {
        if (p.size < 18) return null
        // Fields 3..14 are SVIDs used in solution
        val used = (3..14).mapNotNull { idx -> p.getOrNull(idx)?.toIntOrNull() }
        return Gsa(
            p.getOrNull(15)?.toDoubleOrNull(),  // PDOP
            p.getOrNull(16)?.toDoubleOrNull(),  // HDOP
            p.getOrNull(17)?.toDoubleOrNull(),  // VDOP
            used
        )
    }
    ...
    private fun parseGst(p: List<String>): Gst? {
        ...
        return Gst(
            ...
            p.getOrNull(6)?.toDoubleOrNull(),
            p.getOrNull(7)?.toDoubleOrNull(),
            p.getOrNull(8)?.toDoubleOrNull()
        )
    }
    ...
    private fun decodeLon(raw: String?, hemi: String?): Double? {
        if (raw.isNullOrBlank() || hemi.isNullOrBlank()) return null
        if (raw.length < 5) return null // dddmm...
    ...
    private fun constellationFromType(type: String): Constellation = when {
        ...
        type.startsWith("GN") -> Constellation.UNKNOWN   // mixed; infer per SVID
        else -> Constellation.UNKNOWN
    }

    // SVID-based inference for GN talker; conservative but works well for RS2+
    private fun inferConstellationBySvid(svid: Int): Constellation = when (svid) {
        in 1..32    -> Constellation.GPS
        in 33..64   -> Constellation.SBAS
        in 65..96   -> Constellation.GLONASS
        in 193..200 -> Constellation.QZSS
        in 201..237 -> Constellation.BEIDOU
        in 301..336 -> Constellation.GALILEO
        in 401..437 -> Constellation.BEIDOU
        else        -> Constellation.UNKNOWN
    }
    ...
    companion object TimeUtil {
        ...
        fun buildEpochMillisFromZda(z: Zda): Long? {
            ...
            val year = z.year ?: return null
            val mon = z.month ?: return null
            val day = z.day ?: return null
            return buildCalendarUtc(year, mon, day, t.first, t.second, t.third)
        }
