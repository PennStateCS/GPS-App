...
data class UereTable(
    val internal: Map<RtkStatus, Double> = mapOf(
        RtkStatus.NONE to 4.0, RtkStatus.DGPS to 2.0, RtkStatus.FLOAT to 2.0, RtkStatus.FIX to 2.0
    ),
    val rs2: Map<RtkStatus, Double> = mapOf(
        RtkStatus.NONE to 1.0, RtkStatus.DGPS to 0.5, RtkStatus.FLOAT to 0.25, RtkStatus.FIX to 0.02
    )
) {
